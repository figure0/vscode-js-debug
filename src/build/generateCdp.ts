...
import got from 'got';
import { cdpCustom } from './cdpCustom';
...
type ProtocolType<WithId extends string> = {
  description: string;
  optional?: true;
  deprecated?: true;
  experimental?: true;
} & (WithId extends 'id' ? { id: string } : WithId extends 'name' ? { name: string } : {});

type ProtocolRef = {
  $ref: string;
};

type ProtocolObject<WithId extends string> = ProtocolType<WithId> & {
  type: 'object';
  properties: ReadonlyArray<TopLevelProtocolType<'name'>>;
};

type ProtocolString<WithId extends string> = ProtocolType<WithId> & {
  type: 'string';
  enum?: ReadonlyArray<string>;
};

type ProtocolArray<WithId extends string> = ProtocolType<WithId> & {
  type: 'array';
  items: ProtocolRef;
};

type TopLevelProtocolType<WithId extends string> =
  | ProtocolObject<WithId>
  | ProtocolString<WithId>
  | ProtocolArray<WithId>;
...
interface IProtocolCommand {
  ...
  experimental?: true;
  deprecated?: true;
  parameters?: ReadonlyArray<TopLevelProtocolType<'name'>>;
  returns?: ReadonlyArray<TopLevelProtocolType<'name'>>;
}
...
interface IProtocolDomain {
  ...
  types: ReadonlyArray<TopLevelProtocolType<'id'>>;
...
async function generate() {
  ...
  const domains = [
    ...
    ...cdpCustom.domains,
  ].sort((a, b) => (a.domain.toUpperCase() < b.domain.toUpperCase() ? -1 : 1));
  ...
  function generateType(prop: TopLevelProtocolType<string> | ProtocolRef): string {
    if ('$ref' in prop) {
      return prop.$ref;
    }
  ...
  function appendProps(props: Iterable<TopLevelProtocolType<'name'>>) {
